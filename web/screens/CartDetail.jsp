...
<html lang="en">
    <head>
        ...
        <style>
            ...
            .cart-item .card-detail {
                padding-left: 10px;
            }

            .cart-item .card-detail h5 {
                font-size: 18px;
                margin-bottom: 5px;
            }
            ...
            .button-wrap .btn:hover {
                background-color: #0056b3; /* Example button hover background color */
            }
            ...
            .btn btn-outline-danger btn-sm deleteitems{
                margin-left: 300px;
            }
    ...
    <jsp:include page="../common/header.jsp"></jsp:include>
        <section class="cart padding-large">
            <div class="container">
                <h2 class="titleCart">YOUR CART</h2>
                    ...
                    <div class="cart-item border-bottom padding-small">
                        <div class="row align-items-center">
                            <div class="col-lg-4 col-md-3">
                                <a href="productDetails?id=${cartItem.product.id}" class="text-decoration-none">
                                    <div class="cart-info d-flex gap-2 flex-wrap align-items-center">
                                        <div class="col-lg-5">
                                            <div class="card-image">
                                                <img class="cart_img" src="${pageContext.request.contextPath}/images/${cartItem.product.thumbnail}" alt="Thumbnail">
                                            </div>
                                        </div>
                                        <div class="col-lg-7">
                                            <div class="card-detail">
                                                <h5 class="mt-2">${cartItem.product.description}</h5>
                                                <div class="card-price">
                                                    <span class="price text-primary fw-light" data-currency-usd="${cartItem.price}">$${cartItem.price}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="row d-flex align-items-center">
                                        ...
                                        <div class="product-quantity my-2">
                                            <div class="input-group product-qty align-items-center" style="max-width: 150px;">
                                                <span class="input-group-btn">
                                                    <button type="button" class="bg-white shadow border rounded-3 fw-light quantity-left-minus" data-type="minus" data-field="${cartItem.id}">
                                                        <svg width="16" height="16"><use xlink:href="#minus"></use></svg>
                                                        <span>-</span>
                                                    </button>
                                                </span>
                                                <input type="text" id="quantity-${cartItem.id}" name="quantity-${cartItem.id}" class="form-control bg-white shadow border rounded-3 py-2 mx-2 input-number text-center"
                                                       value="${cartItem.quantity}" min="1" max="1000" required>
                                                <span class="input-group-btn">
                                                    <button type="button" class="bg-white shadow border rounded-3 fw-light quantity-right-plus" data-type="plus" data-field="${cartItem.id}">
                                    ...
                                    <div class="col-md-6">
                                        <div class="total-price">
                                            <span class="money fs-2 fw-light text-primary" >$${cartItem.totalPrice}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-1 col-md-2">
                                <form method="post" action="CartDetailController">
                                    <div class="cart-cross-outline text-center">
                                        <input type="hidden" name="cartItemIdDelete" value="${cartItem.id}">
                                        <button type="submit" name="action" value="delete" class="btn btn-outline-danger btn-sm deleteitems" style="margin-left: 200px;">
                                            DELETE ITEMS
                                        </button>
                                    </div>
                            </form>
            ...
            <div class="cart-totals padding-medium pb-0">
                ...
                <div class="button-wrap d-flex flex-wrap gap-3">
                        <button type="button" onclick="location.reload()" class="btn">Reload Cart</button>
                        <a href="./products" class="btn">Continue Shopping</a>
                        <a href="./CartContactController?user_id=${user.id}" class="btn">Proceed to Checkout</a>
                    </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $(".quantity-right-plus").click(function (e) {
                e.preventDefault();
                var cartItemId = $(this).data("field");
                var currentVal = parseInt($("#quantity-" + cartItemId).val());
                if (!isNaN(currentVal)) {
                    $("#quantity-" + cartItemId).val(currentVal + 1);
                    updateQuantity(cartItemId, currentVal + 1);
                } else {
                    $("#quantity-" + cartItemId).val(0);
                }
            });
            ...
            $(".quantity-left-minus").click(function (e) {
                ...
                if (!isNaN(currentVal) && currentVal > 1) {
                    ...
                    $("#quantity-" + cartItemId).val(1);
                }
            });

            function updateQuantity(cartItemId, newQuantity) {
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "${pageContext.request.contextPath}/CartDetailController", true);
                ...
                var data = "cartItemId=" + encodeURIComponent(cartItemId) +
